  ...
  fileExists(bucket, key, function (err, exists) {
    ...
    if (exists) {
      ...
      s3.listObjects(params, function (err, data) {
        if (err) return cb(err);
        async.forEachLimit(data.Contents, ASYNC_LIMIT, function (item, cb) {
          getFile(bucket, item.Key, localPath, options, cb);
        }, cb);
      });
    }
  });
...
function getFile(bucket, key, localPath, options, cb) {

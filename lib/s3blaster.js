  ...
  var finishedCb = function (err) {
    if (err) {
      ...
      if (err.retryable && entry.retryCount < MAX_RETRIES) {
        console.log('Upload of', entry.relativePath, 'failed, will retry', MAX_RETRIES - entry.retryCount, 'more times.');
        entry.retryCount += 1;
        return putFile(bucket, destPath, options, report, entry, cb);
      }

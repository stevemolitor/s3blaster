...
var S3PutStream = require('./s3-put-stream');
...
function put(srcDir, bucket, destPath, options, cb) {
  ...
  fs.stat(srcDir, function (err, stat) {
    ...
    if (stat.isDirectory()) {
      var dirStream = new DirectoryStream(srcDir);
      var s3Stream = new S3PutStream(bucket, destPath, options, report);
      dirStream.pipe(s3Stream);
      s3Stream.on('end', function () {
        console.log('s3 stream ended');
        cb();
      });
      dirStream.on('end', function () {
        console.log('dir stream ended');
        //cb(null, report);
      });
      dirStream.on('error', function (err) {
        console.error('dir stream error:', err);
      });
      s3Stream.on('error', function (err) {
        console.error('s3 stream error:', err);
      });
    } else { // put single file
      console.error('single file PUT not implemented yet');
      process.exit(1);

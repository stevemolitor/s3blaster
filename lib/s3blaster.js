...
function copy(bucket, prefix, destination, options, cb) {
  if (!options) {
    cb = options;
    options = {};
  }
  ...
  function copyFile(key, destKey, cb) {
    ...
    if (!destKey) {
      destKey = destination + '/' + key.substr(prefix.length + 1);
    }
    var copySource = bucket + '/' + key; // TODO URL encode?
    var params = {CopySource: copySource, MetadataDirective: 'COPY', Bucket: bucket, Key: destKey};
  ...
  fileExists(bucket, prefix, function (err, exists) {
    if (err) return cb(err);

    if (exists) {
      // if this exact file exists, copy it to destination and we're done:
      copyFile(prefix, destination, cb);
    } else {
      var walkOpts = {prefix: prefix};
      walkBucket(bucket, copyFile, walkOpts, function (err) {
        if (err) return cb(err);
        cb(null, total);
      });
    }

...
function put(srcDir, bucket, destPath, options, cb) {
  ...
  var limit = options.limit || ASYNC_LIMIT;
  ...
  var putFn = putFile.bind(null, bucket, destPath, options, report);

  fs.stat(srcDir, function (err, stat) {
    if (err) return cb(err);

    if (stat.isDirectory()) {
      // Read the directory recursively, pushing a task for putting
      // each file onto an async.queue, which will put them to S3.
      //
      // The queue could temporarily drain before the directory
      // listing is finished.  So, wait until both the queue has
      // drained tne listing is done, then call the callback.
      var queueDone = false;
      var listDone = false;

      var queue = async.queue(putFn, limit);
      ...
      local.list(srcDir, queue, function (err) {
        listDone = true;
        if (err) {
          console.error('Error getting directory listing -', err);
          return cb(err);
        }
      });
...
function filteredPut(srcDir, bucket, filterFn, options, cb) {
  ...
  local.list(srcDir, function (err, entries) {
    ...
    async.forEachLimit(filtered, options.limit || ASYNC_LIMIT, putFn, function (err) {
      if (err) return cb(err);
